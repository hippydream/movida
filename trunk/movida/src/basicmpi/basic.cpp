...
: MvdPluginInterface(parent), importDialog(0),

http(0),
startPageUi(0), currentRequest(NoRequest),
...
void MvdBasicMpi::imdbImportEntryPoint()
{
	...
	imdbEngine.name = tr("IMDb movies");
	imdbEngine.queryUrl = QUrl("http://akas.imdb.com/find?s=tt");
	imdbEngine.queryParameter = "q";
	engines.append(imdbEngine);

	importDialog = new MvdwImportDialog(engines, p);
	importDialog->registerResponseHandler(this, "responseHandler");
...
int MvdBasicMpi::responseHandler(const QString& url, QIODevice& response)
{
	Q_ASSERT(importDialog);
	qDebug("looking for matches in %s...", url.toLatin1().constData());
	...
	if (url.contains("title"))
	{
		...
		{	
			importDialog->addMatch(MvdCore::decodeXmlEntities(titleRx.cap(1)));
			return MvdwImportDialog::SingleMatchResponse;
		}
		else return MvdwImportDialog::ErrorResponse;
	}
	return MvdwImportDialog::MultipleMatchesResponse;
}
