...
QStringList MvdImportDialog::Private::buildQueryList(QString s) const
{
    s = s.toLower();
    ...
    d(new Private)
{
    ...
    setPage(Private::ResultsPage, d->resultsPage);
    d->summaryPage = new MvdImportSummaryPage;
    setPage(Private::SummaryPage, d->summaryPage);
...
int MvdImportDialog::nextId() const
{
    ...
    switch (currentId()) {
        case Private::StartPage:
            return Private::ResultsPage;

        case Private::ResultsPage:
            if (field("selectedResultsCount").toInt() == 0)
                return Private::FinalPage;
            return Private::SummaryPage;

        case Private::SummaryPage:
            return Private::FinalPage;
...
bool MvdImportDialog::confirmCloseWizard()
{
    ...
    if (!isBusy()) {
        ...
        switch (currentId()) {
            case Private::ResultsPage:
                msg = tr("Movida is still searching for your query. Are you sure you want to close the wizard?"); break;

            case Private::SummaryPage:
                msg = tr("Movida is still downloading the movie details. Are you sure you want to close the wizard?"); break;

            case Private::FinalPage:
                msg = tr("Movida is still importing your new movies. Are you sure you want to close the wizard?"); break;

            default:
                msg = tr("An operation is still in progress. Are you sure you want to close the wizard?");
        }
    }
