...
: QWidget(parent), d_ptr(new MvdRichTextEditorPrivate(Bottom, this))
{
	init();
}

MvdRichTextEditor::MvdRichTextEditor(ControlsPosition pos, QWidget* parent)
: QWidget(parent), d_ptr(new MvdRichTextEditorPrivate(pos, this))
{
	init();
}
...
void MvdRichTextEditor::init()
{
	...
	QVector<MvdRichTextEditorPrivate::ColorWrapper> colorWrappers(colorNames.size());
	for (int i = 0; i < colorNames.size(); ++i)
		colorWrappers[i] = MvdRichTextEditorPrivate::ColorWrapper(colorNames.at(i));
	qSort(colorWrappers);
	for (int i = 0; i < colorWrappers.size(); ++i)
	{
		const MvdRichTextEditorPrivate::ColorWrapper& cw = colorWrappers.at(i);
