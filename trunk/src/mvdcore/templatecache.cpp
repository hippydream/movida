...
public:
	...
	struct CachedPage {
		CachedPage(quint64 id, const QString& s, MvdMovieCollection* c = 0) : id(id), collection(c), path(s), usage(0) {}
...
MvdTemplateCache& MvdTemplateCache::instance()
{
	if (!mInstance) {
		...
		if (!mInstance) {
			if (mDestroyed)
				throw std::runtime_error("Template Cache: access to dead reference");
			create();
		}
	}
