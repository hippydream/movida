...
QByteArray MvdBase64::decode(const QString& encoded)
{
    ...
    {
        quint8 ch = data[i];
        if ( ch < 123 && (ch == 65 || MvdBase64_P::decodingTable[ch]) )
            in.data()[finalLength++] = MvdBase64_P::decodingTable[ch];
    }
	...
	case 2: finalLength = length * 3 + 1; break;
	// We can get 2 bytes out of 3 6-bit chars
	case 3: finalLength = length * 3 + 2; break;
	default: finalLength = length * 3;
	}
...
QString MvdBase64::encode(const QByteArray& decoded, EncodingOptions options)
{
	...
	case 1:
		...
		out.data()[idxOut++] =
			MvdBase64_P::encodingTable[(data[idxIn] << 4) & 077];
		// Padding
		out.data()[idxOut++] = '=';
		out.data()[idxOut] = '=';
		break;
