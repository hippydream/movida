...
#define MVD_SD_CHECK_ID \
    Q_ASSERT_X(d->nextId != 0xFFFFFFFF, "MvdSharedData", "Available IDs exhausted.");
...
/************************************************************************
    MvdSharedData::Private
 *************************************************************************/

//! \internal
class MvdSharedData::Private
{
public:
    Private();
    Private(const Private &s);
    ~Private();
...
//! \internal
MvdSharedData::Private::Private(const MvdSharedData::Private &s)
{
    data = s.data;
    nextId = s.nextId;
...
//! \internal
void MvdSharedData::Private::logNewItem(const MvdSdItem &item)
{
...
QString MvdSharedData::roleToString(Movida::DataRole role)
{
    ...
    switch (role) {
        ...
        default:
            Q_ASSERT_X(false, "MvdSharedData::roleToString()", "Internal error");
    }
