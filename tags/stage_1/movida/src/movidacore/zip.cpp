...
MvdZip::ErrorCode MvdZip_P::createArchive(QIODevice* dev)
{
	...
	if (!device->isOpen())
	{
		if (!device->open(QIODevice::ReadOnly)) {
			delete device;
			device = 0;
			eLog() << "Unable to open device for writing.";
	...
	MvdZip::CompressionLevel level)
{
	...
	if (!dirOnly)
	{
		...
		{
			eLog() << QString("An error occurred while opening %1")
		...
		else
		{
			...
			{
				actualFile.close();
				eLog() << "Could not initialize zlib for compression";
				delete h;
			...
			do
			{
				...
				if (read < 0)
				{
					actualFile.close();
					deflateEnd(&zstr);
					eLog() << QString("Error while reading %1").arg(file.absoluteFilePath());
					delete h;
				...
				do
				{
					...
					if (device->write(buffer2, compressed) != compressed)
					{
						deflateEnd(&zstr);
						actualFile.close();
						eLog() << QString("Error while writing %1").arg(file.absoluteFilePath());
						delete h;
						return MvdZip::WriteError;
					}
	...
	if ((h->gpFlag[0] & 8) == 8)
	{
		...
		// CRC
		setULong(h->crc, buffer1, MVD_ZIP_DD_OFF_CRC32);
