...
{\
	if (headers != 0)\
{\
	eLog() << "FileCorruptedError zip archive. Some files might be extracted.";\
	ec = headers->size() != 0 ? MvdUnZip::FilePartiallyCorruptedError : MvdUnZip::FileCorruptedError;\
	break;\
}\
		else\
{\
	delete device;\
	device = 0;\
	eLog() << "FileCorruptedError or invalid zip archive";\
