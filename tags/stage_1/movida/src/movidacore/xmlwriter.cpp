...
//! \internal
QString MvdXmlWriter_P::escape(const QString& string) const
{
	QString s = string;
	s.replace("&", "&amp;");
	s.replace(">", "&gt;");
...
//! \internal
QString MvdXmlWriter_P::openTag(const QString& tag, const QHash<QString,QString>& attributes)
{
	QString out = "<";
...
void MvdXmlWriter::setLineBreakType(LineBreakType type)
{
	switch (type)
	{
	case MacLineBreak: d->lineBreak = "\r"; break;
	case WindowsLineBreak: d->lineBreak = "\r\n"; break;
	case UnixLineBreak: d->lineBreak = "\n";
	default: ;
	}
}
